...
# look at missing age at death registration 
dt%>%
    names() 
...
dt[, 
    missing_age := ifelse( !AgeYear %in% 0:122, "Unknown", "Known")
][
        , .(count = .N), by = .(epi_year, missing_age)
]%>%
    #wider
    dcast(epi_year ~ missing_age, value.var = "count") %>%
    mutate( 
        total = Known + Unknown,
...
dt[, 
    sex := ifelse(Sex %>% as.numeric() %in% c(1, 2), "Known", "Unknown")   
    ][
    , .(count = .N), by = .(epi_year, sex)
    ]%>%
    #wider
    dcast(epi_year ~ sex, value.var = "count") %>%
    mutate( 
        total = Known + Unknown,
        pct_unknown = Unknown / total * 100
    ) 
