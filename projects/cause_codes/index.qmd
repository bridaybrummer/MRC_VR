...
- **Grand Total Summary**: Excess mortality aggregated across all causes by year
...
## Grand Total: Excess Deaths by Cause {#grand-total}

This section provides a comprehensive overview of excess mortality across all cause categories.

::: {.callout-note}
## Data Source Note
The grand total calculations combine natural causes from the analysis dataset (`LGH_AnalysisData_2022.feather`) with external causes from the full deaths dataset (`Deaths2022_MRCversionFINAL.feather`). This provides a comprehensive view including both disease-related mortality (with excess calculations) and injury-related mortality (actual counts).

- **Natural causes**: Excess deaths calculated using baseline models
- **External causes**: Actual death counts (road traffic accidents, falls, assault, suicide, etc.)
- Analysis generated by the enhanced `tabulate_excess_grand_total_with_external()` function
:::
...
```{r}
#| label: grand-total-table-combined
#| fig-cap: "Combined excess deaths - natural and external causes"

if (exists("dt_analysis")) {
  # Try to load external causes data for comprehensive analysis
  external_path <- find_data_file("Deaths2022_MRCversionFINAL.feather")
  if (!is.null(external_path)) {
    dt_external <- read_feather(external_path) %>% as.data.table()
  ...
  excess_table %>% flextable_to_rmd()
} else {
  cat("Data not available for grand total analysis")
}
```
...
```{r}
#| label: grand-total-table-natural
#| fig-cap: "Natural causes only - LGH classification"

if (exists("dt_analysis")) {
  # Generate table with natural causes only
  excess_table_natural <- tabulate_excess_grand_total(dt_analysis, years = 2019:2022)
  excess_table_natural %>% flextable_to_rmd()
} else {
  cat("Data not available for natural causes analysis")
}
```
...
```{r}
#| label: grand-total-table-external
#| fig-cap: "External causes only - unnatural deaths"

# Try to load external causes data
external_path <- find_data_file("Deaths2022_MRCversionFINAL.feather")
if (!is.null(external_path)) {
  dt_external <- read_feather(external_path) %>% as.data.table()
  ...
  if (exists("tabulate_external_causes_only")) {
    external_table <- tabulate_external_causes_only(dt_external, years = 2019:2022)
    external_table %>% flextable_to_rmd()
  } else {
    cat("External causes function not available")
  }
} else {
  cat("External causes data not available")
}
```
...
```{r}
#| label: grand-total-plot  
#| fig-cap: "Excess deaths by cause and year - stacked bar chart of top causes"
#| fig-width: 12
#| fig-height: 8
#| fig-align: "center"
#| out-width: "90%"
...
if (exists("dt_analysis")) {
  plot_excess_grand_total(dt_analysis, years = 2019:2022, top_n = 15)
} else {
  cat("Data not available for grand total plot")
}
```
