...
model_list <- list(
    pred_fit_tmb                         = fit_tmb,
    pred_fit_tmb_interactions            = fit_tmb_interactions,
    pred_gam                             = model_gam,
    pred_simple                          = model_simple,
    pred_simple_fourier                  = model_simple_fourier,
    pred_simple_interactions             = model_simple_interactions,
    pred_simple_pop                      = model_simple_pop,
    pred_fourier_interactions_pop_offset = model_fourier_interactions_pop_offset
)

for (nm in names(model_list)) {
    dt_for_model_unfiltered_no_epi53[,
        (nm) := predict(model_list[[nm]], newdata = dt_for_model_unfiltered_no_epi53,
                        type = "response", allow.new.levels = TRUE)
    ]
    dt_for_model_unfiltered_no_epi53[, paste0(nm, "_diff") := count - get(nm)]
}
...
covid_trend_prop_by_year <- data.table(
...
# Apply inverse factor only from the start of the COVID-19 period; pre-pandemic weeks default to 1
national_j_codes[, covid_trend := 1.0]
national_j_codes[
    data.table(week_start = dt_covid_deaths$week_start, trend = inverse_covid_trend_prop),
    on = "week_start",
    covid_trend := i.trend
]
